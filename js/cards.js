...
export const SUITS = [
  { name: 'spade', color: 'black', text: 'บาลี' },
  { name: 'heart', color: 'red', text: 'สันสกฤต' },
  { name: 'club', color: 'black', text: 'สมาส' },
  { name: 'diamond', color: 'red', text: 'สนธิ' },
];

import { WORD_LISTS } from './words.js'; // Make sure to adjust the path
...
export class Card extends EventTarget {
  constructor(number, suit) {
    ...
    this._suit = suit;
    this._visible = false;
    this._word = this._getRandomWordForSuit(suit.name);
  }

  _getRandomWordForSuit(suitName) {
    const words = WORD_LISTS[suitName];
  ...
  get word() { return this._word; }  // Getter for the word
...
export class CardGameUI extends GameUI {
  ...
  constructor(gameDiv, CoreClass) {
    ...
    for (const card of cardArray) {
      ...
      // Create divs for card number and suit text
  ...
  _onCardChanged(card) {
    ...
    for (const suitSpan of div.querySelectorAll('.suit')) {
      suitSpan.textContent = card.word; // Update to show the word
    }
